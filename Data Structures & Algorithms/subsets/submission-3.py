...
only append the subset into our output list when we are past the last index of array

time complexity O(2^n * n) because there can be 2^n subset and the worst subset takes n time to build
Aux space complexity O(n) where n is the depth of the recursion. Since the number of decision is equal to the number of input element, the recursion depth is n.
"""
