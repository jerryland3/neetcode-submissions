...
class Solution:
    def hasPathSum(self, root: Optional[TreeNode], targetSum: int) -> bool:
        def helper(node: Optional[TreeNode], curSum: int):
            # check edge case of null node
            if not node:
                return False
            if isLeaf(node) and (curSum + node.val) == targetSum:
                return True
            
            if helper(node.left, curSum + node.val):
                return True
            if helper(node.right, curSum + node.val):
                return True
