class Solution:
    def permute(self, nums: List[int]) -> List[List[int]]:
        ...
        def helper(usedList: List[bool], index: int):
            if index >= length:
...
recursive case:
    ...
    - restore truth list by setting index j to False and pop from curList

complexity:
    - n! choices and each permutation requires n list append. Time complexity of O(n*n!)
    - curList appending happen in place. Recursion stack is at most n levels. Thus need O(n) aux space and O(n!) output space.
"""
